...
static constexpr std::
    array<std::tuple<char16_t, char16_t, std::string_view, char16_t, std::string_view>, 5>
...
auto to(std::string expr) -> std::string {
  ...
  for (auto c : u16) {
    ...
    if (it != _map.cend()) {
      ...
      if constexpr (Lang == translate::APL or Lang == translate::BQN) {
        ret += std::get<Lang>(*it);
      }
    } else {
