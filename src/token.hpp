...
auto u16_to_token(std::u16string const& s) -> token_t {
    if (std::isdigit(*s.begin())) return token_t::ARRAY;
    if (is_operator(s)) return token_t::OPERATOR;
   ...
   public:
    token(std::u16string str)
      : _type{u16_to_token(str)},
        _value{_type == token_t::ARRAY ? token_variant{to_tensor(str)} : token_variant{str}} {}
